...
Deno.serve(async (req) => {
  // Handle CORS preflight requests by returning a 200 OK response with the appropriate headers.
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders });
  }

  try {
    // ✅ 2. Extrai dados do corpo
    const { fullName, badgeNumber, password } = await req.json();
    if (!fullName || !badgeNumber || !password) {
      return new Response(
        JSON.stringify({ error: 'Dados incompletos fornecidos.' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 }
      );
    }

    // ✅ 3. Captura o Authorization header
    ...
    if (createError) {
      if (createError.message.includes('unique constraint') || createError.message.includes('should be unique')) {
        return new Response(
          JSON.stringify({ error: 'Um usuário com este crachá já existe.' }),
    ...
    // ✅ 9. Garante CORS até em erros inesperados
    return new Response(
