...
const ManageUsersView: React.FC<ManageUsersViewProps> = ({ profiles, setProfiles, currentUserProfile }) => {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
        ...
        setError("Sucesso! Usuário criado. Saia e entre para ver a lista atualizada.");
      ...
      <div className="bg-gray-800 rounded-lg shadow p-4 sm:p-6 border border-gray-700">
        <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
            ...
            <div className="flex items-center gap-2">
                ...
                >
                    <UserPlusIcon /> <span className="hidden sm:inline">Novo</span>
                </button>
            </div>
        </div>

        {error && (
            <div className={`mb-4 text-center p-4 rounded-lg text-sm font-medium ${error.includes('Sucesso') ? 'bg-green-900/40 text-green-400 border border-green-500/30' : 'bg-red-900/40 text-red-400 border border-red-500/30'}`}>
                {error}
            </div>
        )}
        ...
        {paginatedProfiles.length <= 1 ? (
            <div className="text-center py-16 bg-gray-900/50 rounded-xl border border-dashed border-gray-700">
                <p className="text-gray-400">Nenhum funcionário visível além de você.</p>
                <p className="text-xs text-brand-light mt-3 px-4">
                   Se você já criou usuários, eles estão protegidos pelo banco. <br/>
                   <strong>Saia e entre novamente</strong> para ver a lista completa.
                </p>
            </div>
        ) : (
              ...
              <table className="min-w-full divide-y divide-gray-700">
                ...
                <tbody className="bg-transparent divide-y divide-gray-700">
                  {paginatedProfiles.map(person => (
                    <tr key={person.id} className="hover:bg-gray-700/30 transition-colors">
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Novo Usuário">
        <form onSubmit={handleCreateUser} className="flex flex-col gap-4 mt-4">
            <div className="space-y-1">
                <label className="text-xs text-gray-400 font-bold uppercase">Nome Completo</label>
                <input className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-brand-primary outline-none" type="text" placeholder="Ex: João Silva" value={newFullName} onChange={e => setNewFullName(e.target.value)} required />
            </div>
            <div className="space-y-1">
                <label className="text-xs text-gray-400 font-bold uppercase">Nº Matrícula (Login)</label>
                <input className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-brand-primary outline-none" type="text" placeholder="Ex: 12345" value={newEmployeeId} onChange={e => setNewEmployeeId(e.target.value)} required />
            </div>
            <div className="space-y-1">
                <label className="text-xs text-gray-400 font-bold uppercase">Senha Provisória</label>
                <input className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-brand-primary outline-none" type="password" placeholder="Mínimo 6 caracteres" value={newPassword} onChange={e => setNewPassword(e.target.value)} required minLength={6} />
            </div>
            <button type="submit" className="w-full bg-brand-primary hover:bg-brand-secondary text-white font-bold py-3 rounded-xl shadow-lg mt-2 disabled:opacity-50" disabled={isCreatingUser}>
                {isCreatingUser ? 'Criando...' : 'Cadastrar Funcionário'}
            </button>
        </form>
      </Modal>
    </>
